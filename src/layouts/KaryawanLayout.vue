<template>
 <div class="fullscreen-container">
    ...
    <router-link :to="{name: 'HomeKaryawan'}">
      <span class="absolute top-5 text-white sm:text-6xl sm:mt-14 text-3xl ml-8 -mt-2 font-bold  responsive-text">Absensi  <p class="sm:text-lg text-sm  responsive-text">{{ company.nama }}</p></span>
    </router-link>
    <!-- Router Link ke /karyawan/profile (tidak diubah) -->
    <router-link to="/karyawan/profile">
      <button>
        <div class="absolute flex flex-row-reverse -top-0 right-0 mr-8 mt-2 sm:mt-24  transition-all responsive-profile">
